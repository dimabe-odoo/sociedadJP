        ...
        for item in self:
            ...
            if item.picking_type_code == 'incoming':
                ...
                values['picking_type_id'] = self.env['stock.picking.type'].search(
                    [('warehouse_id.id', '=', item.warehouse_id.id), ('sequence_code', '=', 'OUT')]).id
                values['location_id'] = self.env['stock.location'].search([('name', '=', 'Customers')]).id
            if item.picking_type_code == 'outgoing':
                values['name'] = 'IN/' + item.name
                values['picking_type_code'] = 'incoming'
                values['picking_type_id'] = self.env['stock.picking.type'].search(
                    [('warehouse_id.id', '=', item.warehouse_id.id), ('sequence_code', '=', 'IN')]).id
                values['location_id'] = self.env['stock.location'].search([('name', '=', 'Vendors')]).id
